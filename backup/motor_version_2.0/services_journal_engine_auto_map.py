...
TIPO_CUENTA_PATTERNS: Dict[str, Dict] = {
    ...
    "IGV_DEBITO": {
        ...
        # NO usar códigos hardcodeados - buscar solo por tipo y nombre
        "codigos": [],  # Sin códigos hardcodeados - el motor decide dinámicamente
        "nombres": ["igv débito", "igv débito fiscal", "igv por pagar", "débito fiscal"],
        "account_type": AccountType.LIABILITY  # ✅ Correcto: Es Pasivo
    },
    "DETRACCIONES": {
        "codigos": ["12.20", "12.2"],
        "nombres": ["detracciones por cobrar", "detracciones", "detracción"],
        "account_type": AccountType.ASSET
    },
...
def sugerir_cuentas_por_tipo(
    db: Session,
    company_id: int,
    tipo_cuenta: str,
    limit: int = 5
) -> List[Dict]:
    """Devuelve sugerencias rankeadas para un tipo de cuenta.

    No persiste cambios. Está pensado para:
    - UI (mapeo manual asistido)
    - Logs del motor (mostrar qué sugería el sistema)

    Retorna una lista de dicts:
      { "account": Account, "score": float, "reason": str }
    """
    if tipo_cuenta not in TIPO_CUENTA_PATTERNS:
        return []
    ...
    for account in accounts:
        ...
        for codigo in pattern.get("codigos", []) or []:
            if account.code == codigo:
                score += 60.0
                reasons.append(f"código exacto {codigo}")
            elif account.code.startswith(codigo):
                score += 25.0
                reasons.append(f"prefijo {codigo}")

        # Nombre
        name_l = (account.name or "").lower()
        for kw in pattern.get("nombres", []) or []:
            if kw and kw.lower() in name_l:
                score += 15.0
                reasons.append(f"nombre contiene '{kw}'")

        # Tipo contable
        expected_type = pattern.get("account_type")
        if expected_type and getattr(account, "type", None) == expected_type:
            score += 20.0
            reasons.append(f"tipo {expected_type.value if hasattr(expected_type,'value') else expected_type}")

        if score > 0:
            scored.append((account, score, "; ".join(reasons)))

    scored.sort(key=lambda x: x[1], reverse=True)
    top = scored[:max(1, limit)]
    return [{"account": a, "score": float(s), "reason": r} for a, s, r in top]
