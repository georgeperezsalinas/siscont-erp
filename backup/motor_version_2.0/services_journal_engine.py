...
class EngineRunLog:
    """Log estructurado (auditable) de una ejecución del motor.

    Se guarda en JournalEntry.motor_metadata y permite monitoreo desde el módulo del Motor de Asientos.
    """
    ...
    def __init__(self, *, evento_tipo: str, company_id: int, origin: str, fecha: date, glosa: str, datos_operacion: Dict[str, Any]):
        ...
        self.glosa = glosa
        # Guardar un resumen (no todo) para evitar meter PII o payload enorme
        self.datos_operacion_keys = sorted(list(datos_operacion.keys())) if isinstance(datos_operacion, dict) else []
        self.steps: List[Dict[str, Any]] = []
        self.errors: List[Dict[str, Any]] = []
        self.warnings: List[Dict[str, Any]] = []

    def _add(self, level: str, action: str, details: Dict[str, Any] | None = None):
        self.steps.append({
    ...
    def info(self, action: str, details: Dict[str, Any] | None = None):
        self._add("INFO", action, details)

    def warn(self, action: str, details: Dict[str, Any] | None = None):
        self.warnings.append({"ts": datetime.utcnow().isoformat(), "action": action, "details": details or {}})
        self._add("WARN", action, details)

    def error(self, action: str, details: Dict[str, Any] | None = None):
        self.errors.append({"ts": datetime.utcnow().isoformat(), "action": action, "details": details or {}})
        self._add("ERROR", action, details)
...
class MotorAsientos:
    ...
    def __init__(self, uow: UnitOfWork, strict_mode: bool | None = None):
        self.uow = uow
        # Modo estricto: ante mapeos/validaciones inválidas, fallar sin autocorregir.
    ...
    def generar_asiento(
        ...
        # Log contable estructurado (se guarda en motor_metadata del asiento)
        runlog = EngineRunLog(
        ...
        runlog.info("START", {"currency": currency, "exchange_rate": str(exchange_rate)})
        ...
        for regla in sorted(reglas, key=lambda r: r.orden):
            ...
            if not es_valido_mapeo:
                # En un motor contable maduro NO se debe "autocorregir" la configuración en caliente.
                # En su lugar: registrar evidencia + sugerencias y fallar (modo estricto).
                try:
                    from .services_journal_engine_auto_map import sugerir_cuentas_por_tipo
                    sugerencias = sugerir_cuentas_por_tipo(self.uow.db, company_id, regla.tipo_cuenta, limit=5)
                except Exception:
                    sugerencias = []
                ...
                runlog.warn(
                    ...
                    {
                        "tipo_cuenta": regla.tipo_cuenta,
                        "cuenta_actual": {"code": cuenta.code, "name": cuenta.name, "type": getattr(cuenta.type, "value", str(cuenta.type))},
                        "error": error_mapeo,
                        "sugerencias": [
                            {"code": s["account"].code, "name": s["account"].name, "type": getattr(s["account"].type, "value", str(s["account"].type)), "score": s["score"], "reason": s["reason"]}
                ...
                # Modo relajado: continuar (no recomendado en producción), pero queda trazado en el log.
