...
Prioridad:
1. Carga desde archivos CSV de sistema (eventos_contables_base.csv, reglas_contables_base.csv,
   tipo_cuenta_mapeos_base.csv) - igual que plan_base.csv para el plan de cuentas.
2. Si los CSV no existen, usa la lógica hardcodeada legacy.
"""
...
def inicializar_eventos_y_reglas_predeterminadas(db: Session, company_id: int):
    ...
    Prioridad: carga desde archivos CSV de sistema (backend/data/*.csv).
    Si no existen, usa la lógica hardcodeada legacy.
    """
    try:
        from .services_motor_base import load_motor_base_csv
        result = load_motor_base_csv(db, company_id)
        db.commit()
        return result
    except FileNotFoundError:
        pass  # CSV no encontrados, usar lógica legacy
    except Exception:
        raise

    # ===== LÓGICA LEGACY (si no hay CSV) =====
    # ===== EVENTO: COMPRA =====
